...
interface DocItem { id: string; title: string; categoryId: string | null; category?: Category; updatedAt: string }
...
export default function DocumentList({ apiEndpoint, categoryEndpoint, pageTitle, itemTypeLabel, defaultTitle, linkPrefix }: DocumentListProps) {
  ...
  const { data: docs = [], mutate } = useSWR<DocItem[]>(`${apiEndpoint}?search=${encodeURIComponent(search)}${catParam}`, fetcher);
  const { data: categories = [], mutate: mutateCats } = useSWR<Category[]>(categoryEndpoint, fetcher);

  const createDoc = async () => {
    setSaving(true);
    const { error } = await apiFetch(apiEndpoint, {
      method: "POST",
      body: JSON.stringify({ title: newTitle || defaultTitle, categoryId: selectedCat }),
    });
    setSaving(false);
    if (error) { toast(error, "error"); return; }
    toast(`${itemTypeLabel} created`, "success");
    setModalOpen(false);
    setNewTitle("");
    mutate();
  };
  ...
  const deleteDoc = async (id: string) => {
    const { error } = await apiFetch(`${apiEndpoint}/${id}`, { method: "DELETE" });
    if (error) { toast(error, "error"); return; }
    toast(`${itemTypeLabel} deleted`, "success");
    mutate();
  };
  ...
  const createCategory = async () => {
    ...
    if (!name) return;
    const { error } = await apiFetch(categoryEndpoint, {
      method: "POST",
  ...
  const renameCategory = async (id: string, name: string) => {
    ...
    const { error } = await apiFetch(`${categoryEndpoint}/${id}`, {
      method: "PUT",
  ...
  const deleteCategory = async (id: string) => {
    const { error } = await apiFetch(`${categoryEndpoint}/${id}`, { method: "DELETE" });
    if (error) { toast(error, "error"); return; }
  ...
  const moveDocToCategory = async (docId: string, categoryId: string | null) => {
    const { error } = await apiFetch(`${apiEndpoint}/${docId}`, {
      method: "PUT",
    ...
    <>
      ...
      <div className="flex flex-1">
        ...
        <div className="flex-1 p-6">
          {docs.length === 0 ? (
            <EmptyState title={`No ${pageTitle.toLowerCase()} yet`} description={`Create your first ${itemTypeLabel.toLowerCase()} to get started.`} />
          ) : (
            <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-4">
              {docs.map((doc) => (
                ...
                >
                  <Link href={`${linkPrefix}${doc.id}`} className="block">
                  ...
                  {canEdit && (
                    <button
                      ...
                      title={`Delete ${itemTypeLabel.toLowerCase()}`}
                    >
      ...
      {/* New Item Modal */}
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={`New ${itemTypeLabel}`}>
      ...
      <ConfirmDialog
        ...
        title="Delete Category"
        message={`Delete "${confirmDeleteCat?.name}"? ${pageTitle} in this category will become uncategorized. Subcategories will also be deleted.`}
      />
